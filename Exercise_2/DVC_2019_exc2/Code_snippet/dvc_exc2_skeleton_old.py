...
# reference: https://bokeh.pydata.org/en/latest/docs/user_guide/interaction/widgets.html
select_category = Select(title="OP:", value="All", options=category)
...
def datasource_construct(dfnew):
    ...
    df.sort_values(by=['OBSERVATION_DATE']).reset_index(inplace=True)
    ...
    df['OBSERVATION_COUNT_SQRT'] = (np.sqrt((df['OBSERVATION_COUNT'])) *2)
    
     # scale the "OBSERVATION COUNT" column into reasonalbe number serving as the radius when later draw the circle
    # recommend to do the scaling for each item by computing "sqrt(item)*2"
    unscaled_list = df['OBSERVATION_COUNT'].tolist()
    counts = []
    ...
    observers =  sorted(df['NUMBER_OBSERVERS'].tolist())  # extract the column of "NUMBER OBSERVERS" and set it into a list
    ...
    df['TIME_OBSERVATION'] = pd.to_timedelta(df['TIME_OBSERVATIONS_STARTED'])// pd.offsets.Minute(1)
   
    # add the observation ending minutes for drawing the hbar
    df['TIME_OBSERVATIONS_ENDED'] = df['TIME_OBSERVATION'] + df['DURATION_MINUTES']


    data = {'Year': list(df['OBSERVATION_DATE']),
        'Starts': list(df['TIME_OBSERVATION']),
        'Ends': list(df['TIME_OBSERVATIONS_ENDED']),# ending time of the observation
        'RealStarts':list(df['TIME_OBSERVATIONS_STARTED']), # list of column "TIME OBSERVATIONS STARTED", serving for hovertool
        'Duration':  list(df['DURATION_MINUTES']),# list of column "DURATION MINUTES", serving for hovertool
        'Counts': counts, # serves for the size of the circle
        'Name': list(df['COMMON_NAME']),# list of "COMMON NAME", serving for hovertool
        'RealCounts':list(df['OBSERVATION_COUNT']), # list of original "OBSERVATION COUNT", serving for hovertool
        'Observers': observers,
        'Colors': colors
        }
